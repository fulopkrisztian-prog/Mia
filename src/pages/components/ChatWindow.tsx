...
const ChatWindow = () => {
  ...
  const loadChatMessages = async (chatId: string) => {
    try {
      const msgs: any[] = await invoke('get_chat_messages', { chatId });
      ...
      <div className="w-64 bg-slate-900/50 border-r border-slate-800/60 backdrop-blur-xl flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
          {Object.entries(chats).map(([id, name]) => (
            <button
              ...
              className={`w-full flex items-center space-x-3 p-3 rounded-xl transition-all group ${activeChatId === id
                  ? 'bg-blue-600/10 border border-blue-500/20 text-blue-100'
                  : 'hover:bg-slate-800/40 text-slate-400 hover:text-slate-200'
                }`}
            >
              <MessageSquare
                className={`w-4 h-4 shrink-0 ${activeChatId === id ? 'text-blue-400' : 'text-slate-500'
                  }`}
              />
        ...
        <div className="p-4 border-t border-slate-800/50 text-[10px] text-slate-600 flex justify-between items-center">
          <span className="flex items-center">
            <Hash className="w-3 h-3 mr-1" /> v1.2.0
          </span>
          ...
          {messages.map((message) => (
            ...
            >
              <div className={`max-w-2xl group transition-all duration-300`}>
                <div
                  className={`flex items-center space-x-2 mb-2 ${message.sender === 'user' ? 'justify-end' : 'justify-start'
                    }`}
                >
                  <span
                    className={`text-[10px] font-bold uppercase tracking-tighter ${message.sender === 'user' ? 'text-blue-500' : 'text-cyan-500'
                      }`}
                  >
                ...
                <div
                  className={`rounded-2xl p-4 backdrop-blur-md shadow-sm border ${message.sender === 'user'
          ...
          <div className="max-w-3xl mx-auto relative group">
            ...
            <div className="relative flex items-end space-x-2 bg-slate-900/90 border border-slate-700/50 p-3 rounded-2xl shadow-2xl">
              ...
              <button
                ...
                className={`p-2.5 rounded-xl transition-all duration-300 ${inputText.trim() && !isLoading
